...
pro mvn_kp_download_l1_files, instruments=instruments, filenames=filenames, list_files=list_files, $
  start_date=start_date, end_date=end_date, new_files=new_files, $
  update_prefs=update_prefs, only_update_prefs=only_update_prefs, $
  exclude_orbit_file=exclude_orbit_file, debug=debug, sublevel=sublevel,$
  help=help

  if (~keyword_set(sublevel)) then begin
    print, "Please select sublevel a, b, or c"
    print, "For example, 'mvn_kp_download_l1_files, instruments=ngi, sublevel=b'"
    return
  endif

  if (sublevel ne 'a') and (sublevel ne 'b') and (sublevel ne 'c') then begin
    print, "Please select sublevel a, b, or c"
    print, "For example, 'mvn_kp_download_l1_files, instruments=ngi, sublevel=b'"
    return
  endif
  ...
  if keyword_set(help) then begin
    mvn_kp_get_help,'mvn_kp_download_l1_files'
    return
  endif
  ...
  mvn_root_data_dir = mvn_kp_config_file(update_prefs=update_prefs, /l1)
  base_l1_dir = mvn_root_data_dir+'maven'+path_sep()+'data'+path_sep()+'sci'
  ...
  l1_dirs = create_struct( $
    'sta', base_l1_dir+path_sep()+'sta'+path_sep()+'l1'+sublevel+path_sep(), $
    'sep', base_l1_dir+path_sep()+'sep'+path_sep()+'l1'+sublevel+path_sep(), $
    'swi', base_l1_dir+path_sep()+'swi'+path_sep()+'l1'+sublevel+path_sep(), $
    'swe', base_l1_dir+path_sep()+'swe'+path_sep()+'l1'+sublevel+path_sep(), $
    'lpw', base_l1_dir+path_sep()+'lpw'+path_sep()+'l1'+sublevel+path_sep(), $
    'euv', base_l1_dir+path_sep()+'euv'+path_sep()+'l1'+sublevel+path_sep(), $
    'mag', base_l1_dir+path_sep()+'mag'+path_sep()+'l1'+sublevel+path_sep(), $
    'iuv', base_l1_dir+path_sep()+'iuv'+path_sep()+'l1'+sublevel+path_sep(), $
    'acc', base_l1_dir+path_sep()+'acc'+path_sep()+'l1'+sublevel+path_sep(), $
    'ngi', base_l1_dir+path_sep()+'ngi'+path_sep()+'l1'+sublevel+path_sep())
  ...
  for inst_i=0, n_elements(instruments)-1 do begin
    ...
    ;; Get directory of where l1 files are stored on machine
    inst_tag_i = where(tag_names(l1_dirs) eq strupcase(instruments[inst_i]), counter)
    ...
    current_l1_dir = l1_dirs.(inst_tag_i)
    ...
    if not keyword_set(filenames) then begin
      ...
      ;; Always l1 for this procedure
      query_args = [query_args, "level=l1"+sublevel]
      ...
      if (size(filenames, /type) eq 3 || n_elements(filenames) eq 0) then begin
        print, "For instrument: "+instruments[inst_i]+" - No l1"+sublevel+" files found on server for input query"
        print, ""
      ...
      if keyword_set (new_files) then begin
        ...
        local_files = file_basename(file_search(current_l1_dir+path_sep(), pattern))
      ...
      if keyword_set(list_files) then begin
        print, 'For instrument: '+instruments[inst_i]+" - l1 files that would be downloaded: "
        print, "-------------------------------------------------------"
    ...
    if download_bool eq 'yes' then begin
      ...
      print, "Starting download of l1"+sublevel+" file(s)..."
      ...
      for i = 0, nfiles-1 do begin
        ...
        full_path = current_l1_dir + path_sep() + date_path
      ...
      print, strtrim(string(nfiles-nerrs),2)+" total files successfully downloaded to: "+current_l1_dir
      print, ""
