...
;       an array listing the density measurements of particular species to be plotted 
;    rad_species : in, optional, type=intarr(ndims)
;       an array listing the radiance measurements of particular species to be plotted 
;    legends : in, optional, type=strarr(1)
;       either 'vertical' or 'horizontal', defining the orientation of the legend.
;    linear : in, optional, type=byte
;       optional keyword to make plots on a linear scale, instead of the logarithmic default
;    species_expand: in, optional, byte
;       if this keyword is selected, then all the species for a given orbit will be plotted on a single graph
;    profile_expand: in, optional, byte
;       if this keyword is selected, then all the orbits will be combined into a single plot for comparison purposes
;    range: in, optional, byte
;       if selected, this will return the temporal range of the data set without plotting anything
...
pro MVN_KP_IUVS_LIMB, kp_data, density=density, radiance=radiance, profiles=profiles, den_species=den_species, rad_species=rad_species, legend=legend,$
  ...
  ;DEFINE THE SPECIES NAME STRINGS FOR DENSITY AND RADIANCE
    ...
    radiance_names = ['CO2/CO2+','CO','H','O_1304','O_1306','0_2972','C','N','N2','NO']
  ...
  ;CHECK FIRST THAT THE SUPPLIED DATA STRUCTURE INCLUDES PERIAPSE DATA
    ...
    if data_check eq -1 then begin
      print,'The data structure appears to not have any PERIAPSE data. Try again with a different KP data strucutre.'
      goto, finish
    endif
    
  ;INFORM THE USER THAT ALL ORBITS MAY BE PLOTTED
    if keyword_set(profile_expand) eq 1 then begin
      print,'By default, all periapse data will be plotted. This includes orbits ',strtrim(string(kp_data[0].orbit)),' to ',strtrim(string(kp_data[n_elements(kp_data.orbit)-1].orbit))
      ...
      for i=0,n_elements(kp_data)-1 do begin
        for j=0,2 do begin
          if kp_data[i].periapse[j].time_start ne '' then begin
            print,strtrim(string(profile_index),2)+': Orbit #'+strtrim(string(kp_data[i].orbit),2)+', Profile start time '+strtrim(string(kp_data[i].periapse[j].time_start),2)
  ...
  ;INFORM THE USER ABOUT THE CHOICE OF SPECIES TO PLOT
    if keyword_set(den_species) ne 1 then begin
      print,'By default, all species will be plotted. Use the SPECIES keyword to select a subset of plotted species.'
  ...
  ;IF USER HAS DOWNSELECTED SPECIES, CHANGE VARIOUS PARAMTERS TO MATCH
    ...
    if keyword_set(rad_species) then begin
      ...
      radiance_labels = density_names(rad_species-1)
    endif else begin
  ...
  for i=0,n_elements(kp_data) -1 do begin
    for j=0,2 do begin
      if kp_data[i].periapse[j].time_start ne '' then begin
        radiance_data[i,j,*,*] = kp_data[i].periapse[j].radiance[(rad_species-1),*]
        radiance_error[i,j,*,*] = kp_data[i].periapse[j].radiance_err[(rad_species-1),*]
        density_data[i,j,*,*] = kp_data[i].periapse[j].density[(den_species-1),*]
        density_error[i,j,*,*] = kp_data[i].periapse[j].density_err[(den_species-1),*]
        profile_labels[index] = 'Orbit '+strtrim(string(kp_data[i].orbit),2)+', Profile '+strtrim(string(j+1),2)
        index=index+1
      endif
    endfor
  endfor
    ...
    ;DETERMINE HOW MANY PANELS ARE GOING TO BE NEEDED BASED ON EXPANSION OPTIONS
      ...
      if keyword_set(profile_expand) then rows = index
      if keyword_set(profile_expand) and keyword_set(profiles) then rows = n_elements(profiles)
      ...
      if keyword_set(species_expand) then begin
        ...
        if keyword_set(radiance) and keyword_set(density) then columns = 17
        if keyword_set(radiance) eq 0 and keyword_set(density) eq 0 then columns = 17
      endif   
      
      if keyword_set(den_species) then begin
        columns = columns - (7-n_elements(den_species))
      endif
      
      if keyword_set(rad_species) then begin
        columns = columns - (10-n_elements(rad_species))
      endif
    ...
    ;DEFINE THE LINESTYLES AND COLORS FOR EACH SPECIES OR PROFILE
     ...
     ;PLOT
     ; is there a more eloquent way of doing this instead of all the possible different options?
     ...
     ;CREATE THE PLOT WINDOW, SAVING THE OLD ONE IF REQUESTED
      ...
      if rad_plot eq 1 then begin
        ...
        if (keyword_set(species_expand) eq 0) and (keyword_set(profile_expand) eq 0) then begin
          plot,radiance_data[0,0,0,*],altitude,thick=2,xlog=log_option,charsize=2,yrange=[100,220],$
              ...
              for j=0,2 do begin
                if kp_data[i].periapse[j].time_start ne '' then begin
                  ...
                  for k=0,n_elements(rad_species)-1 do begin
                    oplot,radiance_data[i,j,k,*],altitude,thick=2,linestyle=rad_linestyle[k],color=profile_colors[(i*3)+j]
                  endfor
                 endif
        ...
        ;RADIANCE PLOT EXPANDING TO INDIVIDUALLY PLOT ALL SPECIES
         if keyword_set(species_expand) and (keyword_set(profile_expand) eq 0) then begin
          for i=0,n_elements(rad_species)-1 do begin
            check_index = 0
            plot,radiance_data[0,0,i,*],altitude,title=radiance_labels[i],thick=2,xlog=log_option,charsize=1.5,yrange=[100,220],$
                 ytitle='Altitude, km',/nodata,ymargin=y_label_margin
            for j=0,n_elements(kp_data)-1 do begin
              for k=0,2 do begin
                if kp_data[j].periapse[k].time_start ne '' then begin
                  if profile_inclusion[check_index] eq 1 then begin        
                    oplot,radiance_data[j,k,i,*],altitude,thick=2,color=profile_colors[(j*3)+k]
                  endif
                  check_index = check_index+1
                endif
              endfor
        ...
        if keyword_set(profile_expand) and (keyword_set(species_expand) eq 0) then begin
          check_index = 0
          label_index = 0
          for i=0,n_elements(kp_data)-1 do begin
            for j=0,2 do begin
        ...
        if keyword_set(profile_expand) and keyword_set(species_expand) then begin         
          for i=0,n_elements(rad_species)-1 do begin
            check_index = 0
            label_index = 0
            for j=0,n_elements(kp_data)-1 do begin
      ...
      ;IF BOTH RADIANCE AND DENSITY ARE BEING PLOTTED, DRAW A VERTICAL LINE TO SEPARATE THE TWO SIDES OF THE PLOT
      
        ;MID-PLOT LINE IF BOTH RADIANCE AND DENSITY IS INCLUDED
          if keyword_set(species_expand) then begin
            line_marker = float(n_elements(rad_species))/float((n_elements(rad_species)+n_elements(den_species)))
            if rad_plot eq 1 and den_plot eq 1 then begin
              plots,[line_marker,line_marker],[0.,1.],/normal,thick=3,linestyle=1
            endif
          endif
      ...
      if den_plot eq 1 then begin
        ;DEFAULT PLOTTING OF ALL PERIAPSE SCANS, SPECIES, AND PROFILES ON ONE PLOT
         if (keyword_set(species_expand) eq 0) and (keyword_set(profile_expand) eq 0) then begin
          plot,density_data[0,0,0,*],altitude,thick=2,xlog=log_option,charsize=2,yrange=[100,200],$
               ytitle='Altitude,km',/nodata,ymargin =y_label_margin
          check_index = 0
          for i=0,n_elements(kp_data)-1 do begin
            for j=0,2 do begin
              if kp_data[i].periapse[j].time_start ne '' then begin
                if profile_inclusion[check_index] eq 1 then begin
                  for k=0,n_elements(den_species)-1 do begin
                    oplot,density_data[i,j,k,*],altitude,thick=2,linestyle=den_linestyle[k],color=profile_colors[(i*3)+j]
                  endfor
                endif
                check_index = check_index+1
              endif
        ...
        ;DENSITY PLOT EXPANDING TO INVIDUALLY PLOT ALL SPECIES
         if keyword_set(species_expand) and (keyword_set(profile_expand) eq 0) then begin
          for i=0,n_elements(den_species)-1 do begin
            check_index = 0
            plot,density_data[0,0,i,*],altitude,title=density_labels[i],thick=2,xlog=log_option,charsize=1.5,yrange=[100,220],$
                 ytitle='Altitude, km',/nodata,ymargin=y_label_margin
            for j=0,n_elements(kp_data)-1 do begin
              for k=0,2 do begin
                if kp_data[j].periapse[k].time_start ne '' then begin
                  if profile_inclusion[check_index] eq 1 then begin                  
                    oplot,density_data[j,k,i,*],altitude,thick=2,color=profile_colors[(j*3)+k]
                  endif
                  check_index = check_index + 1
                endif
              endfor
    ...
    if keyword_set(species_expand) then begin
      ...
      if rad_plot eq 1 and den_plot eq 1 then begin
        rad_label = (float(n_elements(rad_species))/float((n_elements(rad_species)+n_elements(den_species))))/2.
        den_label = (float(n_elements(rad_species))/float((n_elements(rad_species)+n_elements(den_species))))*1.25
      endif
    endif
      if (rad_plot eq 1) then xyouts, rad_label,.965,'Radiance',/normal,charsize=3
      if (den_plot eq 1) then xyouts, den_label,.965,'Density',/normal,charsize=3
    
   
   ;START A SECOND WINDOW FOR THE PLOT LEGENDS
   if keyword_set(legend) eq 0 then begin
   ...
   ;species labels
    ...
    if den_plot eq 1 then begin
      for i=0,n_elements(den_species) - 1 do begin
        plots,[(den_lab-0.02),(den_lab+0.08)],[.8-(0.08 * i),.8-(0.08*i)],linestyle=den_linestyle[i],thick=2,/normal
        xyouts,(den_lab+0.1),[0.79-(0.08*i)],density_labels[i],charsize=1.5,charthick=1.5,/normal
      endfor
    endif
   
    if rad_plot eq 1 then begin
      for i=0,n_elements(rad_species) -1 do begin
        plots,[(rad_lab-0.02),(rad_lab+0.08)],[.8-(0.08 * i),.8-(0.08*i)],linestyle=rad_linestyle[i],thick=rad_thick[i],/normal
        xyouts,(rad_lab+0.1),[0.79-(0.08*i)],radiance_labels[i],charsize=1.5,charthick=1.5,/normal        
      endfor
    endif
   ...
   for i=0,n_elements(kp_data.orbit) -1 do begin
    for j=0,2 do begin
      if kp_data[i].periapse[j].time_start ne '' then begin
        if profile_inclusion[check_index] eq 1 then begin
          plots,[0.7,0.9],[0.8-((.8/profile_dimensions)*label_index),0.8-((.8/profile_dimensions)*label_index)],thick=2,color=profile_colors[(i*3)+j],/normal
          xyouts,0.7,(0.78-((.8/profile_dimensions)*label_index)),profile_labels[check_index],/normal
          label_index = label_index + 1
        endif
        check_index = check_index+1
      endif
