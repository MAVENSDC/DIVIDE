;+
; :Name: mvn_kp_download_l3_files
;
; :Author: John Martin
;
;
; :Description:
;     Download level 3 data files from the Maven SDC web service for any instrument.
;
;
; :Keywords:
;    instruments: in, required, type=string or strarr
;       Scalar or array of instruments (three letter representations) of l3 data to download/list
;
...
pro mvn_kp_download_l3_files, instruments=instruments, filenames=filenames, list_files=list_files, $
  ...
  if keyword_set(help) then begin
    mvn_kp_get_help,'mvn_kp_download_l3_files'
    return
  endif
  ...
  mvn_root_data_dir = mvn_kp_config_file(update_prefs=update_prefs, /l3)
  base_l3_dir = mvn_root_data_dir+'maven'+path_sep()+'data'+path_sep()+'sci'
  ...
  l3_dirs = create_struct( $
    'sta', base_l3_dir+path_sep()+'sta'+path_sep()+'l3'+path_sep(), $
    'sep', base_l3_dir+path_sep()+'sep'+path_sep()+'l3'+path_sep(), $
    'swi', base_l3_dir+path_sep()+'swi'+path_sep()+'l3'+path_sep(), $
    'swe', base_l3_dir+path_sep()+'swe'+path_sep()+'l3'+path_sep(), $
    'lpw', base_l3_dir+path_sep()+'lpw'+path_sep()+'l3'+path_sep(), $
    'euv', base_l3_dir+path_sep()+'euv'+path_sep()+'l3'+path_sep(), $
    'mag', base_l3_dir+path_sep()+'mag'+path_sep()+'l3'+path_sep(), $
    'iuv', base_l3_dir+path_sep()+'iuv'+path_sep()+'l3'+path_sep(), $
    'acc', base_l3_dir+path_sep()+'acc'+path_sep()+'l3'+path_sep(), $
    'ngi', base_l3_dir+path_sep()+'ngi'+path_sep()+'l3'+path_sep())
  ...
  for inst_i=0, n_elements(instruments)-1 do begin
    ...
    ;; Get directory of where l3 files are stored on machine
    inst_tag_i = where(tag_names(l3_dirs) eq strupcase(instruments[inst_i]), counter)
    ...
    current_l3_dir = l3_dirs.(inst_tag_i)
    ...
    if not keyword_set(filenames) then begin
      ...
      ;; Always l3 for this procedure
      query_args = [query_args, "level=l3"]
      ...
      if (size(filenames, /type) eq 3 || n_elements(filenames) eq 0) then begin
        print, "For instrument: "+instruments[inst_i]+" - No l3 files found on server for input query"
        print, ""
      ...
      if keyword_set (new_files) then begin
        ...
        local_files = file_basename(file_search(current_l3_dir+path_sep(), pattern))
      ...
      if keyword_set(list_files) then begin
        print, 'For instrument: '+instruments[inst_i]+" - l3 files that would be downloaded: "
        print, "-------------------------------------------------------"
    ...
    if download_bool eq 'yes' then begin
      ...
      print, "Starting download of l3 file(s)..."
      ...
      for i = 0, nfiles-1 do begin
        ...
        full_path = current_l3_dir + path_sep() + date_path
      ...
      print, strtrim(string(nfiles-nerrs),2)+" total files successfully downloaded to: "+current_l3_dir
      print, ""
