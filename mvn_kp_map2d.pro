...
pro MVN_KP_MAP2D, kp_data, parameter=parameter, iuvs=iuvs, time=time, $
                  orbit=orbit, list=list, basemap=basemap, colors=colors, $
...
;-km-test: test new basemap routine
;-km-test: Will keep legacy code commented here until vetted
...
;     endif
;     if keyword_set(direct) eq 0 then begin
;       i = image( mapimage, position=[.15,.1,.85,.9], $
;                  image_dimensions=[360,180] )
;       mp = map( map_projection, limit = map_limit, /box_axes, $
;                 position=[.15,.1,.85,.9], /current )
;       mp.limit = map_limit
;       plot_color = "White"
;     endif
;    endif else begin
...
;        mp = map('Equirectangular', limit = map_limit, /box_axes, $
;                 position=[.15,.1,.85,.9],/current)
