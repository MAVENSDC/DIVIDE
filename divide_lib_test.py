...
def download_files(filenames=None, 
    ...
    if (iuvs == True):    
        data_dir = os.path.join(mvn_root_data_dir,'maven','data','sci','kp','iuvs')
    else:
    ...
    print "Your request will download a total of: "+len(s)+" files with an approx total size of: "+estimated_size+" MBs."
    ...
    for f in s:
        i = i+1
        create_dir_if_needed(f, data_dir)
        get_file_from_site(f, public, data_dir)
...
def get_file_from_site(filename, public, data_dir):
    ...
    with open(os.join.path(data_dir,filename), "wb") as code:
