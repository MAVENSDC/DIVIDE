...
pro mvn_kp_iuvs_nalt_struct, filename, nalts
  ...
  ;
  nalts = {periapse_rad:32, periapse_den:53, c_l_limb:-1, c_l_high:150, c_e_limb:-1, c_e_high:-1}
  ;
...
;            nalts.periapse_rad = fix( nalt_line[1], type=2 )
;          endif
;          if strmatch( temp, '*N_ALT_DEN_BINS*' )then begin
;            nalt_line = strsplit( temp, '=', /extract )
;            nalts.periapse_den = fix( nalt_line[1], type=2 )
;          endif
